...
public class IA {
    ...
    public void setNiveau(int DEPTH) {
        this.DEPTH = DEPTH;
    }
    ...
    public int getColumn(final String[][] game) {
        ...
        analyzeFuturePosition(root, DEPTH, game, Const.COMPUTER);
    ...
    private void analyzeFuturePosition(Node node, final int depth, final String[][] game, final String player) {
        ...
        if (playerWin(game, Const.PLAYER)) {
            node.setEstimation(-MAX - MAX);
            node.setDepth(depth);
            return;
        }
    ...
    private int estimateGame(final String[][] game) {
        final int computer_estimation = gameValue(game, Const.COMPUTER) + winInOneShot(game, Const.COMPUTER) + winInTwoShots(game, Const.COMPUTER);
        final int human_estimation = gameValue(game, Const.PLAYER) + winInOneShot(game, Const.PLAYER) + winInTwoShots(game, Const.PLAYER);
    ...
    private String getFuturePlayer(final String player) {
        return (player.equals(Const.COMPUTER)) ?  Const.PLAYER : Const.COMPUTER;
    }
